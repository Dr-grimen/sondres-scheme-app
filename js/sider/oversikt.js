...
export function Oversikt({ tilstand }) {
  ...
  useEffect(() => { lastAlle(['papir', 'hendingar', 'tankar', 'skann', 'nivaa', 'selskap', 'penger', 'polymarket']).then(setD); }, []);
  ...
  return html`
    ...
    <section class="kort"><h2>Papirresultat · siste måledag <small>${pg && pg.dag ? `måledag ${pg.dag}` : 'papirhandel · ingen ekte pengar er flytta'}</small></h2>
      ${!pg || pg.i_dag == null ? html`<${Tom} tekst="ingen måling enno – papirboka har ikkje køyrt" />` : html`
      <div class="tal">
        <${Flis} tekst=${`${pg.i_dag > 0 ? '+' : ''}${usdKr(pg.i_dag)} USD`} l="på måledagen" kl=${gronRaud(pg.i_dag)} />
        <${Flis} tekst=${pg.i_dag_pst == null ? '–' : pst(pg.i_dag_pst, 2)} l="måledagen, prosent" kl=${gronRaud(pg.i_dag_pst)} />
        <${Flis} tekst=${`${usdKr(pg.eigenkapital)} USD`} l=${`papirbok · start ${usdKr(pg.kapital_start, 0)}`} kl="cyan" />
      ...
      <div class="tal" style="margin-top:10px">
        <${Flis} tekst=${`${pg.n_vinn || 0} / ${(pg.n_vinn || 0) + (pg.n_tap || 0)}`} l=${`lukka handlar i pluss på måledagen (av ${pg.n_handlar_i_dag || 0} handlar)`} />
      ...
      ${pg.beste || pg.verste ? html`<p class="stille" style="margin-top:10px">
        ${pg.beste ? html`<b>Beste på måledagen:</b> ${pg.beste.eigedel || pg.beste.symbol} ${usdKr(pg.beste.realisert)} USD` : null}
      ...
      ${(pg.meklarar || []).filter((m) => !m.demo).length ? html`<p class="stille">Sist lesen meklarstatus (konfigurert ekte): ${pg.meklarar.filter((m) => !m.demo).map((m) => `${m.vising} ${fmt(m.sum, 0)} ${m.valuta} · ${alderTekst(m.ts)}`).join(' · ')}</p>` : null}
      ${pg.dagleg_stopp ? html`<p class="feil">DAGLEG STOPP er på: ingen nye handlar i dag.</p>` : null}
      ${pg.kill_switch ? html`<p class="feil">KILL-SWITCH er på i papirboka. Må nullstillast for hand.</p>` : null}
      ${(pg.manglar || []).length ? html`<p class="stille">${pg.manglar.join(' · ')}</p>` : null}
      <p class="stille">Tala over kjem frå den interne papirboka, ikkje ei demokontomåling. <a href="#/meklarar">Sjå kontostatus</a>.</p>`}
    </section>

    <${PolymarketKort} snapshot=${d && d.polymarket && d.polymarket.snapshot} compact=${true} />
    ...
    <section class="kort"><h2>Papirboka <small>intern simulering · historikken er bevart</small></h2>
      <div class="tal">
        <${Flis} v=${p.eigenkapital} l="eigenkapital, USD" kl="cyan" />
        <${Flis} tekst=${pnl == null ? '–' : fmt(pnl, 0)} l="siste måledag, USD" kl=${pnl > 0 ? 'gron' : (pnl < 0 ? 'raud' : '')} />
